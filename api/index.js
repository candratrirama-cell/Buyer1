...
module.exports = async (req, res) => {
    ...
    const { question, userId, apikey } = req.body;
    if (!question) return res.status(400).json({ error: 'Question is required' });
    if (!apikey) return res.status(403).json({ answer: 'APIKEY wajib diisi.', sources: [] });
    ...
    try {
        ...
        usersSnap.forEach(child => {
            const u = child.val();
            if (u && u.apikey === apikey) apiOwner = { id: child.key, ...u };
        });

        if (!apiOwner) {
            return res.status(403).json({ answer: 'APIKEY tidak valid.', sources: [] });
        }

        if ((apiOwner.limit || 0) <= 0) {
            return res.status(403).json({ answer: 'Limit APIKEY anda habis.', sources: [] });
        }
        ...
        await db.ref(`users/${apiOwner.id}`).update({ limit: (apiOwner.limit || 0) - 1 });
